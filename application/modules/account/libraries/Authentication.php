...
class Authentication {
	...
	function sign_in($account_id, $remember = FALSE)
	{
		...
		$this->CI->load->model('4v_account/accounter');
		
		// get user data (id, group, etc..) 
		$data =$this->CI->accounter->get_user_data($account_id);	
      ...
      $this->CI->session->set_userdata($data);
		...
		redirect($this->CI->config->item('main_page_after_sign_in'));
	}
